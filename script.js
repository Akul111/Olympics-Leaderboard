...
$(document).ready(function () {
    ...
    $("form").submit(function (event) {
        ...
        event.preventDefault();
        $("#countryCompareButton").prop("disabled", true);

        $.get("criterion.php", function (responseData) {

            responseData2 = JSON.parse(responseData);
            let len = responseData2.length;
            let insertedHtml = "<table class='table table-striped table-light ' id = 'allCountryTable'>";
            insertedHtml += "<tr>" +
                "<th class='firstColumn' id='firstRow'>Rank</th>" +
                "<th>Country</th>" +
            ...
            for (let i = 0; i < len; i++) {
                ...
                insertedHtml += "<tr>" +
                    "<td class='firstColumn'>" + (i + 1) + "</td>" +
                    "<td >" + countryname + "</td>" +
                    "<td >" + gold + "</td>" +
                    "<td >" + noOfCyclist + "</td>" +
                    "<td>" + Math.round(age) + "</td></tr>";
            }
            ...
            if ($("#simpleCompareTable").css("display") !== "none") {
            ...
            $("#leaderboardButton").click(function () {
                $("#bigTable").css("display", "table");
                $("#simpleCompareTable").css("display", "none");
                $("#leaderboardButton").prop("disabled", true);
                $("#countryCompareButton").prop("disabled", false);

            })


            $("#countryCompareButton").click(function () {
                $("#simpleCompareTable").css("display", "table");
                $("#bigTable").css("display", "none");
                $("#countryCompareButton").prop("disabled", true);
                $("#leaderboardButton").prop("disabled", false);
